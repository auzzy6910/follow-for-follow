...
export default function Header({ onMenuToggle }) {
  ...
  return (
    <header className="h-16 bg-dark-800 border-b border-dark-600 flex items-center px-3 sm:px-4 lg:px-6 gap-2 sm:gap-3 lg:gap-4 shrink-0">
      ...
      <div className={`flex-1 max-w-xl ${mobileSearchOpen ? 'block' : 'hidden md:block'}`}>
        <div className="relative">
          <Search size={18} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500" />
          <input
            type="text"
            placeholder="Search users, tribes & niches..."
            className="w-full bg-dark-700 border border-dark-500 rounded-xl pl-10 pr-9 py-2.5 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-green-accent/50 transition-colors"
            autoFocus={mobileSearchOpen}
          />
          {mobileSearchOpen && (
            <button
              onClick={() => setMobileSearchOpen(false)}
              className="md:hidden absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-white p-1"
