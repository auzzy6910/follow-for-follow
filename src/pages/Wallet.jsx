...
export default function Wallet() {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-5 sm:space-y-6">
      ...
      <div className="bg-gradient-to-r from-dark-800 to-dark-700 border border-blue-accent/20 rounded-2xl p-4 sm:p-6">
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="w-14 h-14 sm:w-20 sm:h-20 rounded-2xl bg-blue-accent/10 flex items-center justify-center shrink-0">
            <Coins className="text-blue-accent w-8 h-8 sm:w-10 sm:h-10" />
          </div>
        </div>
        <div className="grid grid-cols-3 gap-3 sm:gap-4 mt-4 sm:mt-6">
          <div className="bg-dark-900/50 rounded-xl p-3 text-center">
            <p className="text-blue-accent font-bold text-lg">+{creditHistory.filter(t => t.type === 'earned').reduce((s, t) => s + t.amount, 0).toLocaleString()}</p>
              ...
              <ul className="mt-3 space-y-1.5">
                {plan.features.map((f, j) => (
                  <li key={j} className="text-gray-400 text-xs flex items-center gap-1.5">
                    <div className="w-1 h-1 rounded-full bg-blue-accent" /> {f}
                  </li>
                ))}
              </ul>
